    ...
    while( (await ns.getServerMoneyAvailable( server)!= serverInfo.moneyMax|| await ns.getServerSecurityLevel( server)!= serverInfo.minDifficulty) && !startedHacking){ // While where not at Full health & full cash
        ...
        */
        while( await ns.getServerSecurityLevel( server)!= serverInfo.minDifficulty&& !startedHacking){ // Force full grow before progressing;
            ...
            let weakenTimeNeeded = await ns.Formulas.hackingformulas.weakenTime( serverInfo, player);
        ...
        if( growThreadsNeeded>= 1){
            ...
            let growTimeNeeded= await ns.Formulas.hackingformulas.growTime( targetInfo, player);
...
export async function main( netScript) {
    ...
    await netScript.writePort(1, 'sigma-cosmetics');
    ns= netScript;
    let _loop= 0;
    let _waited= 0;
    ...
    do {
        ...
        // Least -> Most time til;
        let start = Date.now();
        let timeToHack= await ns.Formulas.hackingformulas.hackTime( targetInfo, player);
        let timeToGrow= await ns.Formulas.hackingformulas.growTime( targetInfo, player);
        let timeToWeaken= await ns.Formulas.hackingformulas.weakenTime( targetInfo, player);
        ...
        */
        let hackThreads= Math.ceil( await ns.getServerMaxMoney( target)* .25)/ ns.Formulas.hackingformulas.hackPercent( targetInfo, player);
