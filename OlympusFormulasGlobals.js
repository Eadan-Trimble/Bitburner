...
async function generateServerlists() { 
    ...
    */
    ...
    for( let i= 0; i< _slaveList.length; i++){
    ...
    while( (await ns.getServerMoneyAvailable( server)!= serverInfo.moneyMax|| await ns.getServerSecurityLevel( server)!= serverInfo.minDifficulty) && !startedHacking){ // While where not at Full health & full cash
        ...
        */
        while( await ns.getServerSecurityLevel( server)!= serverInfo.minDifficulty&& !startedHacking){ // Force full grow before progressing;
            ...
            let weakenThreadsNeeded = Math.ceil(( await ns.getServerSecurityLevel( server)- serverInfo.minDifficulty)/ await ns.weakenAnalyze(1, 1));
            let weakenTimeNeeded= await ns.formulas.hacking.weakenTime( serverInfo, player)
            ...
            ns.printf( "Time till weaken: %s m %s s", new Date( weakenTimeNeeded).toLocaleTimeString('en-US'));
        ...
        if( growThreadsNeeded>= 1){
            ...
            let growTimeNeeded= await ns.formulas.hacking.growTime( serverInfo, player);
            ...
            ns.printf("Time till grow: %s ", new Date( growTimeNeeded).toLocaleTimeString('en-US'));
...
export async function main( netScript) {
    netScript.clearPort(1);
    await netScript.writePort(1, 'silver-helix');
    ns= netScript;
    let _loop= 0;
    let _waited= 0;
    ...
    do {
        ...
        if(! startedHacking|| await ns.getServerMoneyAvailable( target)/ await ns.getServerMaxMoney( target)< .89){
            await prepareServer( await getTarget(), Servers.slaves, startedHacking, targetInfo);
            startedHacking= true;
            await ns.sleep( 1500);
        };
        // Batch target;
        // Least -> Most time til;
        let start = Date.now();
        let timeToHack= await ns.formulas.hacking.hackTime( targetInfo, player);
        let timeToGrow= await ns.formulas.hacking.growTime( targetInfo, player);
        let timeToWeaken= await ns.formulas.hacking.weakenTime( targetInfo, player);
        ...
        */
        let hackThreads= Math.ceil( await ns.getServerMaxMoney( target)* .25)/ ns.formulas.hacking.hackPercent( targetInfo, player);
        ...
        timeTillNextBatch= ( finish- start)/ 20;
